...
function Action() {
  ...
  useEffect(() => {
    ...
    if (searchRedux) {
      ...
      ApiAll.getPopular("now_playing", { page: page })
        .then((repo) => {
          setListNow([...listNow, ...repo?.data?.results]);
          setLoading(false);
        })
        .catch((err) => {
          console.log(err);
  ...
  return (
    <>
      <div className="grid grid-cols-5 gap-6 relative mb-8">
        ...
        <div
          className=" rounded-xl text-center absolute left-[50%] -bottom-16 translate-x-[-50%] cursor-pointer"
